...
const runCommonCurriculumFromCommonWords = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }) => {
		...
		const res = await app.fetch(
			new Request(
				`${origin}/api/admin/languages/${data.id}/run-common-curriculum-from-common-words`,
				{
					method: "POST",
					headers: forwardedAdminApiHeaders(request, { jsonBody: json }),
					body: json,
...
const runCurriculumTestabilityTrimRetry = createServerFn({ method: "POST" })
	.inputValidator((data: { id: string }) => data)
	.handler(async ({ data }) => {
		const request = getRequest()
		if (!request) {
		...
		const res = await app.fetch(
			new Request(
				`${origin}/api/admin/languages/${data.id}/run-curriculum-testability-trim-retry`,
				{
					method: "POST",
					headers: forwardedAdminApiHeaders(request, { jsonBody: json }),
					body: json,
		...
		const body = (await res.json().catch(() => ({}))) as { error?: string; jobId?: string }
		if (!res.ok) {
			throw new Error(body.error ?? `Re-check testability failed (${res.status})`)
		}
		return { success: true, jobId: body.jobId ?? null }
	})
...
function AdminLanguagesPage() {
	...
	>({})
	const [clozePromptPreviewByLang, setClozePromptPreviewByLang] = useState<
		Record<string, ClozePromptPreviewResult>
	>({})
	...
	async function handlePreviewClozePrompt(langId: string) {
		...
		setNotice(null)
		setClozePromptPreviewByLang((m) => {
			const { [langId]: _removed, ...rest } = m
			return rest
		})
	...
	async function handleCurriculumTestabilityTrimRetry(id: string) {
		setNotice(null)
		setRunningCurriculumTrim(id)
		try {
			const out = await runCurriculumTestabilityTrimRetry({ data: { id } })
			setNotice({
				kind: "ok",
				text: out.jobId
					? `Re-check non-testable queued — ${out.jobId.slice(0, 8)}… Runs full cloze pipeline (generate + validate) per row; promotes only on success. See job output and metadata.curriculumTestabilityTrimRetry after completion.`
					: "Re-check testability queued.",
			})
		} catch (e) {
			setNotice({
				kind: "err",
				text: e instanceof Error ? e.message : "Re-check testability failed",
			})
		} finally {
			setRunningCurriculumTrim(null)
		}
	...
	return (
		<div className="p-6 space-y-6 relative">
			...
			<div className="text-sm text-muted-foreground space-y-1">
				...
				<p className="text-xs">
					Recent <strong className="text-foreground font-medium">ingestion jobs</strong> appear
					under each row (Output / Skip → next / Cancel / Retry / Re-run).{" "}
					<strong className="text-foreground font-medium">Kaikki vocab</strong> only runs after you
					have lemmas: finish <strong className="text-foreground font-medium">Common words</strong>{" "}
					or add words in{" "}
					<Link
					...
					<strong className="text-foreground font-medium">Kaikki vocab</strong> (chains words gloss
					cleanup). Optionally{" "}
					<strong className="text-foreground font-medium">Trim testability</strong> (LLM: drop rare
					tenses/archaic senses), or use{" "}
					<strong className="text-foreground font-medium">LLM vocabulary</strong>. Or use{" "}
							...
							<div key={lang.id} className="bg-background">
								<div className="grid grid-cols-[1fr_90px_90px_100px_minmax(14rem,1fr)] gap-4 items-center px-4 py-2.5 hover:bg-muted/30 transition-colors">
									...
									<span className="text-right tabular-nums">
										...
										>
											AI {lang.aiWordCount.toLocaleString()} · Curr{" "}
											{lang.commonWordCount.toLocaleString()}
										</span>
									</span>
									...
									<div className="px-4 py-3 bg-muted/20 border-t border-border/70">
										...
										<div className="px-4 pb-3 bg-muted/20 border-t border-border/40">
											...
											<div className="mt-4 pt-3 border-t border-border/40 space-y-2">
												...
												<p className="text-[11px] text-muted-foreground leading-relaxed">
													Runs a single LLM call with the same system prompt, user prompt, and JSON
													schema as <span className="font-mono">Generate clozes</span>. Optional
													fields mirror a real row&apos;s{" "}
													<span className="font-mono">curriculumUnit</span> (gloss → definitions,
													tags, rank, unit type). Nothing is written to the database. Each returned
													row shows <span className="font-mono">llmRaw</span> (what the model
													returned, same as stored{" "}
													<span className="font-mono">sourceCandidates</span>) and{" "}
													<span className="font-mono">afterValidation</span> (the same normalization
													the batch job applies before scoring — includes filling blanks when the
													model put <span className="font-mono">____</span> in{" "}
												...
												<div className="flex flex-wrap items-end gap-2 gap-y-2">
													<div className="flex flex-col gap-0.5 min-w-[8rem] flex-1">
														...
														<input
															...
															onChange={(e) =>
																patchClozePromptTest(lang.id, { lemma: e.target.value })
															}
													...
													<div className="flex flex-col gap-0.5 flex-1 min-w-[10rem]">
														...
														<input
															...
															onChange={(e) =>
																patchClozePromptTest(lang.id, { gloss: e.target.value })
															}
													...
													<div className="flex items-center gap-1.5">
														<div className="flex flex-col gap-0.5 w-14">
															...
															<input
																...
																onChange={(e) =>
																	patchClozePromptTest(lang.id, { rank: e.target.value })
																}
												...
												<div className="flex flex-col gap-0.5">
													...
													<input
														...
														onChange={(e) =>
															patchClozePromptTest(lang.id, { tags: e.target.value })
														}
														className="h-7 rounded border border-input bg-background px-2 text-xs font-mono focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
													/>
												</div>
												<div className="flex flex-wrap items-center gap-3">
													...
													<Button
														...
														disabled={
															clozePromptPreviewLoading === lang.id || generatingClozes === lang.id
														}
														onClick={() => void handlePreviewClozePrompt(lang.id)}
													>
														{clozePromptPreviewLoading === lang.id
															? "Running…"
															: "Run prompt preview"}
													</Button>
												</div>
												{clozePromptPreviewByLang[lang.id]?.summary ? (
													<div className="rounded border border-border/70 bg-background/80 p-2 space-y-2 text-[11px]">
														<p className="font-mono text-muted-foreground tabular-nums">
															LLM returned{" "}
															{clozePromptPreviewByLang[lang.id].summary?.returned ?? "—"}; after
															normalize {clozePromptPreviewByLang[lang.id].summary?.usable ?? "—"}{" "}
															usable; selection would keep{" "}
														...
														<details className="group">
															...
															<pre className="mt-1 max-h-40 overflow-auto rounded bg-muted/40 p-2 text-[10px] leading-snug whitespace-pre-wrap break-words">
																{JSON.stringify(
																	clozePromptPreviewByLang[lang.id].unitJson,
																	null,
																	2,
																)}
															</pre>
														...
														<details className="group">
															...
															<pre className="mt-1 max-h-64 overflow-auto rounded bg-muted/40 p-2 text-[10px] leading-snug whitespace-pre-wrap break-words">
																{JSON.stringify(
																	clozePromptPreviewByLang[lang.id].candidatePreviewRows,
																	null,
																	2,
																)}
															</pre>
														...
														<details className="group">
															...
															<pre className="mt-1 max-h-48 overflow-auto rounded bg-muted/40 p-2 text-[10px] leading-snug whitespace-pre-wrap break-words">
																{JSON.stringify(
																	clozePromptPreviewByLang[lang.id].rawLlmCandidates,
																	null,
																	2,
																)}
															</pre>
														...
														<details className="group">
															...
															<pre className="mt-1 max-h-64 overflow-auto rounded bg-muted/40 p-2 text-[10px] leading-snug whitespace-pre-wrap break-words">
																{JSON.stringify(
																	clozePromptPreviewByLang[lang.id].usableCandidates,
																	null,
																	2,
																)}
															</pre>
														...
														<details className="group">
															...
															<pre className="mt-1 max-h-64 overflow-auto rounded bg-muted/40 p-2 text-[10px] leading-snug whitespace-pre-wrap break-words">
																{JSON.stringify(
																	clozePromptPreviewByLang[lang.id].selectedCandidates,
																	null,
																	2,
																)}
															</pre>
