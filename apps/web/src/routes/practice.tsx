...
function PracticePage() {
  ...
  const practiceLanguagesInvalid = !practiceNativeId || !practiceTargetId || practiceNativeId === practiceTargetId;
  ...
  const activeReportId = reportKeyForQuestion ? reportIdByQuestionKey[reportKeyForQuestion] : undefined;

  const hintLabel =
    question?.hintSource === "parallel" ? `Hint (${nativeLabel ?? "your language"})` : "Hint (dictionary gloss)";
        ...
        <Card>
          <CardHeader className="pb-4">
            <CardDescription>
              {isGuest ? (
                <>
                  You&apos;re not signed in. Pick your language and the one you&apos;re studying (they must differ).{" "}
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label htmlFor="practice-target">Let&apos;s learn</Label>
                <Select
            ...
            {practiceLanguagesInvalid && (
              <p className="text-sm text-muted-foreground">Choose two different languages above to start.</p>
            )}
          </CardContent>
        </Card>
            ...
            <CardHeader>
              ...
              <CardDescription>
                {vocabMode === "ASSESSMENT" ? (
                  <>
                    We&apos;ll find your vocabulary level via binary search. Correct = you know it, wrong = you
                    don&apos;t.
                  </>
                  ...
                  <>
                    Fill the blank in{" "}
                    <span className="font-medium text-foreground">{targetLabel ?? "your target language"}</span> using
                    the hint.
                  </>
                )}
              </CardDescription>
            ...
            <CardContent className="space-y-4">
              {assessmentDone ? (
                <div className="py-8 text-center space-y-4">
                  ...
                  <p className="text-xs text-muted-foreground">{assessmentDone.wordsTestedCount} questions answered</p>
                ...
                <div className="w-full max-w-4xl mx-auto space-y-4">
                  ...
                  <div className="space-y-2 rounded-lg border border-border/80 bg-muted/30 p-4 min-h-22">
                    ...
                    <p className="text-sm leading-relaxed text-muted-foreground text-pretty">{question.hintText}</p>
                  </div>
                  <div className="space-y-2">
                    <Label
                  ...
                  <div className="flex flex-wrap items-center gap-2 min-h-10">
                    ...
                    <Button
                      ...
                      variant="outline"
                      disabled={reportBusy}
                      className={activeReportId ? "text-foreground ml-auto" : "text-muted-foreground ml-auto"}
                  ...
                  {reportMsg ? <output className="text-xs text-muted-foreground block">{reportMsg}</output> : null}
                </div>
              )}
              {feedback && <p className="text-sm text-muted-foreground border-t border-border pt-4">{feedback}</p>}
            </CardContent>
          </Card>
        )}
      </div>
...
function ClozePrompt({ promptText, inlineHint }: { promptText: string; inlineHint: string | null }) {
  ...
  return (
    <>
      {before}
      <span className="underline underline-offset-4 decoration-brand/60 text-brand font-semibold">{inlineHint}</span>
      {after}
    </>
  );
}
