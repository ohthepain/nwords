...
export function WordDetailDialog(props: WordDetailDialogProps) {
  ...
  function adminHeaderAndStats(word: WordPanelWord, knowledge: WordPanelKnowledge | null | undefined) {
          ...
          <DialogTitle className="flex flex-wrap items-center gap-3">
            ...
            {word.alternatePos.length > 0 && (
              <div className="flex flex-wrap items-center gap-1.5 basis-full">
                <span className="text-[10px] text-muted-foreground uppercase tracking-[0.12em]">Also</span>
        ...
        {body ? (
          <div className="space-y-2">
            ...
            {loadingSentences ? (
              ...
              <p className="text-xs text-muted-foreground py-4 text-center">No sentences linked to this word.</p>
            ) : (
              <div className="space-y-2 max-h-[40vh] overflow-y-auto">
                {sentences.map((s) => (
                  <div key={s.id} className="rounded-md border border-border px-3 py-2 text-sm space-y-1">
                    <p>{s.text}</p>
