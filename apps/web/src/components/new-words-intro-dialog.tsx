...
export const NEW_WORDS_INTRO_VARIANTS = [
	"aurora",
	"sunrise",
	"forest",
	"confetti",
...
export function NewWordsIntroDialog({
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent
				className={cn(
					"max-w-md gap-0 overflow-hidden p-0 sm:max-w-md border-2",
			...
			>
				<div className="space-y-4 p-6 pt-8">
					<DialogHeader className="space-y-3 text-center sm:text-center">
						...
						<DialogTitle
							className={cn("text-2xl font-bold tracking-tight", variantTitleClass(variant))}
						>
							{introKind === "intro_backlog"
								? "Batch these new lemmas?"
								: "Ready for some new words?"}
						</DialogTitle>
							...
							{introKind === "intro_backlog" ? (
								canBeginPractice ? (
									<>
										...
										untested, clozable lemmas in the active band — enough that we batch the next few
										instead of dropping you on one at random. Up to{" "}
										<span className="font-semibold text-foreground">{practiceLemmaCount}</span> in
										rank order (same short pass as column practice), then normal Build again.
									</>
								) : showCuratedUnlinkedCopy ? (
									<>
										This batch doesn&apos;t have cloze-ready sentences yet. Stay on general Build,
										or fix sentence links for these lemmas.
									</>
								) : (
									<>
										None of these lemmas have cloze-ready sentences yet. Stay on general Build for
										now.
									</>
								)
							) : canBeginPractice ? (
								practiceLemmaCount === words.length ? (
									<>
										Congratulations! Column {columnIndex + 1} on your heatmap is open. You&apos;ll
										practice <span className="font-semibold text-foreground">{words.length}</span>{" "}
										fresh words at the edge of your territory — short, focused, and right at your
										level.
									</>
								) : (
									<>
										...
										the frontier.{" "}
										<span className="font-semibold text-foreground">{practiceLemmaCount}</span> have
										cloze-ready sentences now — we&apos;ll run through those first (in column
								...
								<>
									...
									<span className="font-semibold text-foreground">{words.length}</span> lemmas at
									the edge of your territory. They have curated sentence IDs, but none are wired for
									cloze yet (missing or mismatched sentence links, removed sentences, or language
									filters). A listed new-words session can&apos;t start for this batch. Use general
									Build vocabulary for now, or fix links in the corpus pipeline.
								</>
							) : (
								<>
									Column {columnIndex + 1} on your heatmap is open with{" "}
									<span className="font-semibold text-foreground">{words.length}</span> lemmas at
									the edge of your territory. None of them have cloze-ready sentences yet, so a
									listed new-words session can&apos;t start for this batch. Use general Build
									vocabulary for now, or try again after more sentences ship for these ranks.
								</>
							)}
						</DialogDescription>
					</DialogHeader>
						...
						<ul className="space-y-1 text-sm">
							...
							{words.length > 24 ? (
								<li className="text-xs text-muted-foreground pt-1">
									+{words.length - 24} more in this batch
								</li>
							) : null}
						</ul>
					</div>
				...
				<DialogFooter className="border-t border-border/50 bg-background/50 p-4 sm:justify-center gap-2">
					...
					<Button
						type="button"
						size="sm"
						disabled={busy || !canBeginPractice}
