...
function devSelection(vocabMode: VocabMode, kind: DevSelection["kind"], summary: string): DevSelection {
...
async function handleNewWordsNext(
  ...
  if (ordered.length === 0) {
    return c.json(
      {
        error: "no_question_available",
        message: "This New words session has no word list. End it and start again from the Build heatmap.",
      },
      404,
    );
  }
...
async function handleBuildNext(
  ...
  if (graphVisibleWordIds.length === 0) {
    return c.json(
      {
        error: "no_question_available",
        message: "No vocabulary in the graph range for your level. Complete an assessment or try another mode.",
      },
      404,
    );
  }
...
export const testRoute = new Hono<OptionalAuthEnv>()
  ...
  .post("/sessions", zValidator("json", startTestSessionBodySchema), async (c) => {
    ...
    if (user) {
      ...
      if (hasOne && !hasPair) {
        return c.json({ error: "Provide both nativeLanguageId and targetLanguageId for a practice pair." }, 400);
      }
        ...
        return c.json(
          {
            ...
            message: "Set native and target language in settings, or pass nativeLanguageId and targetLanguageId.",
          },
          400,
        );
      }
  ...
  .get("/sessions/:id/next", async (c) => {
    ...
    if (forceSentenceId) {
      ...
      if (!targetWordId) {
        return c.json({ error: "no_question_available", message: "No testable word found for this sentence." }, 404);
      }
      const resolved = await resolveClozeWithHint({
        wordId: targetWordId,
        nativeLanguageId: langs.nativeLanguageId,
      ...
      if (!resolved.ok) {
        return c.json({ error: "no_question_available", message: "Could not build a cloze for this sentence." }, 404);
      }
      return c.json({
        wordId: resolved.wordId,
        lemma: resolved.lemma,
        ...
        if (langs) {
          ...
          if (!synonymFeedback) {
            ...
            if (targetWord) {
              ...
              const allDifferent = guessResults.length > 0 && guessResults.every((g) => g.pos !== targetWord.pos);
  ...
  .get("/sessions/:id/upcoming", async (c) => {
    ...
    const peekWordId =
      peekWordIdRaw && /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(peekWordIdRaw)
        ? peekWordIdRaw
        : undefined;
