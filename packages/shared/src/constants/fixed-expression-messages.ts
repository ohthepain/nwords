...
// ── Seed data: Swedish fixed expressions ───────────────

export const SWEDISH_FIXED_EXPRESSION_RULES: FixedExpressionRule[] = [
	{
		trigger: "tycker",
		required: "om",
		invalid: ["än", "på"],
		messageId: "fixed_expression",
		params: { expression: "tycka om", meaning: "to like" },
	},
	{
		trigger: "tänker",
		required: "på",
		invalid: ["om"],
		messageId: "fixed_expression",
		params: { expression: "tänka på", meaning: "to think about" },
	},
	{
		trigger: "bra",
		required: "på",
		invalid: ["i"],
		messageId: "fixed_expression",
		params: { expression: "vara bra på", meaning: "to be good at" },
	},
]

// ── Lookup by target-language code ─────────────────────

const RULES_BY_LANGUAGE: Record<string, FixedExpressionRule[]> = {
	sv: SWEDISH_FIXED_EXPRESSION_RULES,
}

/**
 * Return the fixed-expression rules for a given target language,
 * or an empty array if none exist yet.
 */
export function getFixedExpressionRules(targetLanguageCode: string): FixedExpressionRule[] {
	return RULES_BY_LANGUAGE[targetLanguageCode] ?? []
}
